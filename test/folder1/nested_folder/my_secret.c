/*  Determine whether a list of lists contains a diagonal line of 
    identical values
    lists_diagonal.c

    Written by Hayton Lam (z5358949) on 14/04/2021

    Program which determines whether an identical valued diagonal exists.
*/
...
int check_valid(struct node *head, int index, int value);
...
int check_valid(struct node *head, int index, int value){
    ...
    while (head != NULL && counter <= index) {
        ...
        if (head->data == value && counter == index){        
            return 1;
        }
...
int has_diagonal(struct list_node *head) {
    ...
    while (head != NULL) {
        ...
        if (check_valid(head->my_list, index, value) == 0) {
            return 0;
        }
